...
Because $A_i^{(\ell)}$ depends only on weights in layers $1$ through~$\ell$, a first-order Taylor expansion gives
%
...
\end{equation}
%
where the approximation is first-order in~$\eta$ (the remainder is $O(\eta^2)$, involving second derivatives of the activities with respect to the weights).  For piecewise-linear activations such as ReLU, the expansion is exact whenever the step is small enough that no unit's activation status changes.
%
...
\fi
If a biological circuit performs \emph{any} form of gradient descent on its synaptic weights---even an approximate or noisy variant---and operates in a regime where $\Phi^{(\ell)}$ is approximately diagonal-dominant, then the observable activity changes must satisfy Eq.\,5.  The per-neuron scaling $\Phi^{(\ell)}_{ii}$ is unknown and may vary across neurons, but the \emph{sign} of each neuron's activity change is pinned: because $\Phi^{(\ell)}_{ii}>0$, every neuron must move in the direction that locally reduces the loss, i.e.\ $\operatorname{sign}(\Delta A_i^{(\ell)}) \approx \operatorname{sign}(-\partial \loss/\partial A_i^{(\ell)})$.  Note that if $\Phi^{(\ell)}_{ii}$ varies substantially across neurons, the population vector $\Delta\vA^{(\ell)}$ will not be parallel to $-\nabla_{\!\vA^{(\ell)}}\loss$ but rather to $-\mathrm{diag}(\Phi^{(\ell)}_{ii})\,\nabla_{\!\vA^{(\ell)}}\loss$; the two directions coincide only when $\Phi^{(\ell)}_{ii}$ is approximately uniform.  A concrete experimental protocol follows: record neural population activity across learning trials, estimate $\partial \loss/\partial A_i^{(\ell)}$ from a task-fitted model, and check whether the measured $\Delta A_i^{(\ell)}$ covaries positively with $-\partial \loss/\partial A_i^{(\ell)}$ across neurons.  The unknown $\Phi^{(\ell)}_{ii}$ absorbs per-neuron scaling, so the test checks the sign pattern and rank-order correlation, not the exact magnitudes.  A failure of this sign alignment would rule out not just backpropagation but \emph{any} weight-space gradient rule in a regime where diagonal dominance is expected to hold.  However, the test requires estimating $\partial\loss/\partial A_i^{(\ell)}$, which depends on the downstream computation from layer~$\ell$ to the output; in practice this must come from a fitted model.  A negative result is therefore ambiguous: it could indicate that the brain does not perform gradient descent, or that the downstream model used to estimate the loss gradient is wrong.  The test is most informative when the downstream model can be independently validated.
...
\end{equation}
By (i) and (iii) each summand has the same distribution, with mean of order $n$, so to leading order $\Phi^{(\ell)}_{ii}\approx\ell\cdot\kappa\cdot n$ for a He-determined constant $\kappa$.  Treating $\sigma'_i$, $g_i\coloneqq\partial \loss/\partial A^{(\ell)}_i$, and the realisation of $\lVert J\rVert^2$ as i.i.d.\ across $(\ell,i)$, and $\ell$ as approximately uniform on $\{1,\dots,D\}$:
