...
\section*{Layer-local kernel descent}

The global kernel $\Theta$ couples every neuron to every other neuron across all layers.  But the feedforward structure permits a per-layer formulation that is both exact and more local.  For neurons $i,k$ in the same layer~$\ell$, define
%
...
\end{equation}
%
---the same Gram-matrix construction as the global $\Theta$ (Eq.\,4), restricted to neurons within layer~$\ell$.  Then
%
\begin{equation}\label{eq:layer-kernel}
  \boxed{\;\Delta\vA^{(\ell)} \;=\; -\eta\,\Phi^{(\ell)}\,\nabla_{\!\vA^{(\ell)}}L\;,}
\end{equation}
%
where $\nabla_{\!\vA^{(\ell)}}L$ is the full backpropagated loss gradient at layer~$\ell$ (the total derivative, not the partial output error used in the global equation).

\paragraph{Why the per-layer equation is exact.}
Neuron~$i$ in layer~$\ell$ depends only on weights in layers $1$ through~$\ell$, so $\Delta A_i^{(\ell)} = -\eta\sum_{\alpha\in\text{layers}\,1..\ell} (\partial A_i^{(\ell)}/\partial W_\alpha)\,(\partial L/\partial W_\alpha)$.  In a feedforward network, the effect of any such weight on the loss passes entirely through the activities of layer~$\ell$, so $\partial L/\partial W_\alpha = \sum_{k\in\text{layer}\,\ell} (\partial L/\partial A_k^{(\ell)})\,(\partial A_k^{(\ell)}/\partial W_\alpha)$.  Exchanging the sums yields Eq.\,\ref{eq:layer-kernel} with the kernel $\Phi^{(\ell)}$ defined in Eq.\,\ref{eq:phi-def}.  The backpropagated gradient $\nabla_{\!\vA^{(\ell)}}L$ acts as a \emph{sufficient statistic}: it compresses everything downstream layers contribute to the weight updates that affect layer~$\ell$, so the per-layer equation is not an approximation but an identity.
